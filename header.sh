...
install_basic_features(){
    ...
    if check_bluetooth_hardware; then
        ...
        sudo pamcman -R bluez bluez-utils
    fi
...
check_bluetooth_hardware() {
    if lsmod | grep -q bluetooth; then
        echo "Bluetooth hardware detected (bluetooth kernel module loaded)."
        return 0
    else
        echo "No Bluetooth hardware detected (bluetooth kernel module not loaded)."
        return 1
    fi
}